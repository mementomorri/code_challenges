...
def expmod(b: int, exp: int, m: int):
    if exp == 0:
        ...
        return (expmod(b, exp // 2, m) * expmod(b, exp // 2, m)) % m
    else:
...
if __name__ == "__main__":
    ...
    # In the last range it already takes more than 7.3 seconds to calculate first number.
    # The biggest problem here is that we calculate same thing twice: expmod(b, exp // 2, m)).
    # It leads to loosing all benefits of fast exponentiation algorithm.
